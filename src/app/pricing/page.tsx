...
const packages = [
  {
    name: "Minimum",
    description: "Entry-level package for secure in-person testing.",
    badge: "Permanent Tier",
    icon: Shield,
    iconColor: "text-slate-400 dark:text-slate-500",
    features: {
      assessments: "Core 4 included",
      delivery: "In-person only",
      seats: "Up to 5 HM seats",
      versions: "v1.0.0 access",
  ...
  {
    name: "Professional",
    description: "Our standard plan for remote, hybrid, and custom testing.",
    badge: "Most Popular",
    icon: Zap,
    iconColor: "text-sky-500 dark:text-sky-400",
    features: {
      assessments: "Core 4 + additions",
      delivery: "Remote & Hybrid",
      seats: "Up to 20 HM seats",
      versions: "Latest versions (up to v1.5.0)",
      upgrades: "Purchased separately",
      loyalty: "Standard renewal pricing",
  ...
  {
    name: "Grandfather",
    description: "Launch edition plan with ultimate platform upgrades.",
    badge: "Go-Live Exclusive",
    icon: Award,
    iconColor: "text-amber-500 dark:text-amber-400",
    features: {
      assessments: "Core 4 + all additions",
      delivery: "Remote & Hybrid",
      seats: "Unlimited seats",
      versions: "Latest versions + custom packs",
      upgrades: "All free during year 1",
      loyalty: "30-40% off future upgrades",
...
const featureMatrix = [
  {
    ...
    rows: [
      { name: "Default core assessments (SJA, Typing, Prioritisation, Call Simulation)", minimum: true, professional: true, grandfather: true },
      { name: "Access to additional platform-released assessments", minimum: false, professional: "Option to purchase", grandfather: true },
      { name: "Maximum assessment version access", minimum: "v1.0.0", professional: "v1.5.0", grandfather: "Latest + Custom packs" },
      { name: "Custom content packs (bespoke scenarios, wording)", minimum: false, professional: "Option to purchase", grandfather: true },
    ],
  },
  {
    category: "Delivery & Security",
    rows: [
      { name: "In-person controlled venue delivery", minimum: true, professional: true, grandfather: true },
      { name: "Remote proctored sessions", minimum: false, professional: true, grandfather: true },
      { name: "Hybrid session links & scheduling", minimum: false, professional: true, grandfather: true },
      { name: "Organisation-level soft lock controls", minimum: true, professional: true, grandfather: true },
    ],
  },
  {
    category: "Seats & Administration",
    rows: [
      { name: "Included Hiring Manager seats", minimum: "5 seats", professional: "20 seats", grandfather: "Unlimited" },
      { name: "Candidate portal interface", minimum: true, professional: true, grandfather: true },
      { name: "Hiring Manager portal & grading reports", minimum: true, professional: true, grandfather: true },
      { name: "Client dashboard & custom settings", minimum: true, professional: true, grandfather: true },
    ],
  },
  {
    category: "Commercials & Upgrades",
    rows: [
      { name: "Licensing commitment", minimum: "Annual contract", professional: "Annual contract", grandfather: "Annual contract" },
      { name: "Payment terms", minimum: "Paid monthly via Direct Debit", professional: "Paid monthly via Direct Debit", grandfather: "Paid monthly via Direct Debit" },
      { name: "Free upgrades during active contract year", minimum: false, professional: false, grandfather: true },
      { name: "Permanent loyalty discount on future upgrades", minimum: false, professional: false, grandfather: "30% – 40% discount" },
    ],
  },
];
...
const faqs = [
  {
    ...
    answer: "Clients commit to a 12-month contract period for stability and lock in their pricing tier. However, payments are collected on a monthly basis via direct debit instead of requiring a large upfront annual payment.",
  },
  {
    question: "How do Hiring Manager seats work?",
    answer: "A Hiring Manager seat is required for any team member who needs to create campaigns, view candidate grading reports, or unlock sessions. Minimum includes 5 seats, Professional includes 20, and Grandfather includes unlimited seats.",
  },
  {
    question: "Can we upgrade our tier mid-contract?",
    answer: "Yes, you can request an upgrade at any time from your Client Dashboard. Mid-contract upgrades are calculated as a pro-rata adjustment for the remainder of your annual license term.",
  },
  {
    question: "What is the Grandfather tier loyalty benefit?",
    answer: "Clients who lock in our Grandfather tier during our platform go-live window receive all upgrades free during their first contract year. Upon renewal, they transition to a Professional tier structure but retain a permanent loyalty discount of 30-40% on all subsequent feature or custom content purchases.",
  },
];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#020202] text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
      ...
      <section className="relative overflow-hidden py-24 text-center">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6">
          ...
          <h1 className="font-display text-5xl font-medium tracking-tight text-slate-900 dark:text-white md:text-7xl">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg font-light leading-relaxed text-slate-600 dark:text-slate-400">
            Select a package structure that aligns with your operational cadence. All contracts feature annual licence terms billed via monthly direct debit. No upfront billing surprises.
          </p>
        </div>
      </section>
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="rounded-2xl bg-slate-100 p-3 dark:bg-white/[0.03]">
                      <Icon className={cn("h-6 w-6", pkg.iconColor)} />
                    </div>
                    <div>
                      <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{pkg.name}</h3>
                      <span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">{pkg.badge}</span>
                    </div>
                  </div>
                </div>
                ...
                <ul className="mt-8 space-y-4 flex-grow">
                  {Object.entries(pkg.features).map(([key, value]) => (
                    <li key={key} className="flex items-start gap-3 text-xs leading-relaxed text-slate-600 dark:text-slate-400">
                      <Check className="mt-0.5 h-4.5 w-4.5 shrink-0 text-sky-500 dark:text-sky-400" />
                      <div>
                        <span className="capitalize font-medium text-slate-800 dark:text-slate-300">{key}:</span> {value}
                      </div>
                    </li>
                  ))}
                </ul>
      ...
      <section className="mx-auto max-w-[1440px] px-6 pb-24">
        <div className="rounded-3xl border border-slate-200/80 bg-white p-8 dark:border-white/5 dark:bg-[#080c16]">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            A granular breakdown of standard, grandfather, and entry-level contract entitlements.
          </p>
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-white/5">
                  ...
                  <th className="py-4 font-semibold text-slate-900 dark:text-white w-1/5">Grandfather</th>
                </tr>
              </thead>
              <tbody>
                {featureMatrix.map((category) => (
                  <tr key={category.category} className="contents">
                    <tr className="bg-slate-50/80 dark:bg-white/[0.01]">
                      <td colSpan={4} className="py-3 px-4 text-xs font-mono uppercase tracking-wider text-sky-600 dark:text-sky-400 font-semibold">
                        {category.category}
                      </td>
                    </tr>
                    {category.rows.map((row) => (
                      <tr key={row.name} className="border-b border-slate-100 dark:border-white/5 last:border-none hover:bg-slate-50/35 dark:hover:bg-white/[0.01]/30">
                        <td className="py-4 px-4 font-light text-slate-700 dark:text-slate-300 leading-normal">{row.name}</td>
                        <td className="py-4 px-4 text-slate-600 dark:text-slate-400">
                          {typeof row.minimum === "boolean" ? (
                            row.minimum ? <Check className="h-4.5 w-4.5 text-sky-500" /> : <X className="h-4.5 w-4.5 text-slate-300 dark:text-slate-600" />
                          ) : (
                            <span className="text-xs font-medium">{row.minimum}</span>
                          )}
                        </td>
                        <td className="py-4 px-4 text-slate-600 dark:text-slate-400">
                          {typeof row.professional === "boolean" ? (
                            row.professional ? <Check className="h-4.5 w-4.5 text-sky-500" /> : <X className="h-4.5 w-4.5 text-slate-300 dark:text-slate-600" />
                          ) : (
                            <span className="text-xs font-medium">{row.professional}</span>
                          )}
                        </td>
                        <td className="py-4 px-4 text-slate-600 dark:text-slate-400">
                          {typeof row.grandfather === "boolean" ? (
                            row.grandfather ? <Check className="h-4.5 w-4.5 text-sky-500" /> : <X className="h-4.5 w-4.5 text-slate-300 dark:text-slate-600" />
                          ) : (
                            <span className="text-xs font-medium text-sky-500 font-semibold">{row.grandfather}</span>
                          )}
                        </td>
                      </tr>
                    ))}
        ...
        <div className="grid gap-12 lg:grid-cols-3">
          <div>
            ...
            <p className="mt-4 text-slate-500 dark:text-slate-400 font-light leading-relaxed">
              Clear, transparent answers about our licensing terms, direct debit payment cadence, and pricing tiers.
            </p>
          </div>
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={handleInputChange}
                        required
                        placeholder="Emergency Services Agency"
                        className="rounded-xl h-11"
                      />
                    </div>
                    <div className="space-y-2">
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground pt-4">
                      A sales representative will contact you at <span className="font-semibold">{formData.email}</span> within 1 business day.
                    </p>
                  </div>
                  <Button
                    variant="outline"
        ...
        <div className="mx-auto max-w-[1440px] px-6">
          <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-4 lg:gap-8 mb-12">
            ...
            <div className="flex flex-col gap-4">
              ...
              <Link href="/#disciplines" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Disciplines
              </Link>
              <Link href="/#workflow" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Hiring Workflow
              </Link>
              <Link href="/pricing" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Pricing
              </Link>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <Link href="/auth/register?mode=register" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Get Started
              </Link>
              <Link href="/auth/register?mode=login" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Log in
              </Link>
              <Link href="/privacy-policy" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Privacy Policy
              </Link>
              <Link href="/terms-conditions" className="text-sm font-light text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
                Terms
              </Link>
            </div>
          </div>
