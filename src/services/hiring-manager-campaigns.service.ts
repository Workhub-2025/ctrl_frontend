...
function getSessionPendingInvites(candidateSessions: RawCandidateSession[] = []) {
  return candidateSessions
    .filter((session) => {
      ...
      const hasUser = Boolean(getLinkedCandidateUser(session.users_permissions_users));
...
function normalizeAssessmentSession(session: RawAssessmentSession): HiringManagerSessionListItem {
  ...
  return {
    ...
    candidates: candidates.map((candidateSession) => {
      const user = getLinkedCandidateUser(candidateSession.users_permissions_users);
      const name = resolveCandidateDisplayName(user, candidateSession);
...
function normalizeHmCandidateReport(raw: RawHmCandidateReport): HiringManagerCandidateReport {
  ...
  const candidateUser = getLinkedCandidateUser(raw.candidate ?? null);
  const sessionFields = raw.session ?? {};
  const candidateName = resolveCandidateDisplayName(candidateUser, sessionFields);
