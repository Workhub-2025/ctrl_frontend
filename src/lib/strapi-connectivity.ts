...
export type StrapiConnectivityIssue = {
  code: "missing" | "private" | "unreachable";
  message: string;
...
export function validateStrapiApiUrlForServerless(): StrapiConnectivityIssue | null {
  ...
  if (isPrivateOrLocalHost(hostname)) {
    return {
      code: "private",
      message: `STRAPI_API_URL points to a private/local host (${hostname}) that Vercel cannot reach. Use the public backend URL: https://be.ctrl-assess.co.uk/api`,
