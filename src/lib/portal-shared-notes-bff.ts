...
export type PortalSharedNotesRole = "client" | "hiring_manager";

export async function handleSharedCandidateNotesGet(
  _request: NextRequest,
  params: Promise<{ id: string }>,
  role: PortalSharedNotesRole,
) {
  try {
    const { recruitment } = await requireFirebaseRecruitmentSession(role);
...
export async function handleSharedCandidateNotesPost(
  request: NextRequest,
  params: Promise<{ id: string }>,
  role: PortalSharedNotesRole,
) {
  try {
    ...
    const { session, context: actor, recruitment } =
      await requireFirebaseRecruitmentSession(role);
    const rateLimited = await rejectRateLimitedMutation(request, {
      scope: role === "client" ? "client:candidate-note:create" : "hm:candidate-note:create",
