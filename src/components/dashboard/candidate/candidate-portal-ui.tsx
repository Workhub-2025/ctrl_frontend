    ...
    <header className="relative mb-8 overflow-hidden rounded-2xl border border-border/55 bg-card/40 p-6 shadow-sm dark:border-white/5 dark:bg-[#0b1329]/25 sm:p-7">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3.5">
          <span
            className="mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary shadow-sm"
            aria-hidden="true"
          >
...
export function CandidatePanel({
  ...
  children: ReactNode;
  className?: string;
  accent?: keyof typeof panelAccentClassNames;
}) {
  return (
    <div
      className={cn(
...
export function CandidateStatTile({
  ...
  return (
    <CandidatePanel className={toneClass}>
...
export function CandidateStatusBanner({
  ...
  const tone =
    status === "Completed" || status === "Progressed"
      ...
      : status === "Soft Locked"
        ...
        : status === "Unsuccessful"
          ? "border-rose-500/25 bg-rose-500/5 text-rose-800 dark:text-rose-200"
          : status === "In Progress"
            ? "border-blue-500/25 bg-blue-500/5 text-blue-800 dark:text-blue-200"
            : "border-primary/20 bg-primary/5 text-foreground";

  return (
    <div className={cn("rounded-xl border px-4 py-3.5 text-sm leading-relaxed", tone)}>
